...
pub trait IsMessage {
    ...
    fn get_option(&self, tp: options::CoAPOption) -> Option<&LinkedList<Vec<u8>>> {
        self.get_message().get_option(tp)
    }
    ...
    fn get_message_id(&self) -> u16 {
         self.get_message().header.get_message_id()
    }
